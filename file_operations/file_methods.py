    ...
    def move_old_models_to_archive(self):
        ...
        try:
            if not os.path.isdir(path):
                os.makedirs(path)
                self.file_object = open(self.file_path, 'a+')
                self.logger_object.log(self.file_object,"Successfully created the Archive directory for old models !!")
            self.file_path = "Training_Logs/ModelLoader.txt"
            self.file_object = open(self.file_path, 'a+')
            self.logger_object.log(self.file_object,"Archive Directory already exists!!")
            ...
            self.logger_object.log(self.file_object, "Successfully Moved the old models to the Archive directory !! ")
            self.file_object.close()
        except Exception as e:
            self.file_object = open(self.file_path, 'a+')
            self.logger_object.log(self.file_object,"Exception occurred while moving the old models to the directory !!.Exceptions message!!%s"%e)
            self.file_object.close()
            raise e
